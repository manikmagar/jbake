    ...
    public int renderCategories(Set<String> categories, String categoriesPath) throws Exception {
        ...
        for (String category : categories) {
            try {
            	...
            	//For categories listing, remove the index.html to generate /categories/categoryName/ instead of /categories/categoryName/index.html
            	generatedCategories.put(category, uri.replace("/index.html", "/"));
                renderedCount++;
            } catch (Exception e) {
                errors.add(e.getCause().getMessage());
            }
